...
10. **Reasoning workflows:** Treat planning, reflection, critique, debate, research synthesis, and delegated reasoning as methods rather than model features. On the free router, use short staged prompts, explicit intermediate artifacts, bounded subagent fan-out, and verifiable outputs; never require hidden chain-of-thought, a particular reasoning model, or unlimited context.
11. **Plugin compatibility:** Treat plugins and plugin-provided skills as optional capability providers. Check whether a plugin is installed, enabled, configured, and exposes the required tool before invoking it. If it is unavailable or a routed model rejects its tool schema, use an available native tool or explain the exact manual step. Do not install, enable, reconfigure, or grant a plugin credentials merely because a skill mentions it.
