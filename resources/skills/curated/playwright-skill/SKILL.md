...
Imported from [lackeyjb/playwright-skill](https://github.com/lackeyjb/playwright-skill) under the bundled MIT license.

## Codex compatibility and safety

Use shell execution sessions for the Node runner. In environments without a display, set `PW_HEADLESS=true`. Prefer the built-in controlled-browser capability when the user explicitly asks to interact with their live browser session. Do not reuse authenticated sessions, submit forms, make purchases, publish content, or change remote data unless the user's request clearly authorizes that exact action. Redact credentials and cookies from scripts, logs, and artifacts.
